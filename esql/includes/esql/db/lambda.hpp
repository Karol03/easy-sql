...
#define DEFINED_ARGS_WHERE(__QUERY_BODY, ...) \
    ::esql::db::lambda::Query([__VA_ARGS__]() -> ::esql::db::lambda::SemanticTree \
    { \
        auto __manager = ::esql::db::lambda::FieldManager{}; \
        { [&]() -> std::shared_ptr<::esql::db::lambda::semantic::OperationResultBase> __QUERY_BODY (); } \
...
#define WHERE0(__QUERY_BODY, ...) \
    ::esql::db::lambda::Query([&]() -> ::esql::db::lambda::SemanticTree \
    { \
        auto __manager = ::esql::db::lambda::FieldManager{}; \
        { [&]() -> std::shared_ptr<::esql::db::lambda::semantic::OperationResultBase> __QUERY_BODY (); } \
        return __manager.result(); \
 ...
 */
#define Field(__ClassField) ::esql::db::lambda::Field(__manager, \
                                                       __ClassField##NameGetter(), \
                                                       ::esql::utils::TypeOf<decltype(__ClassField)::ValueType>().type(), \
