...
class Database : public IDatabaseConnection,
                 ...
                 public std::enable_shared_from_this<Database<SQLType>>
{
private:
...
public:
    inline static std::shared_ptr<Database<SQLType>> createInstance()
    {
        auto database = Database<SQLType>{};
        return std::make_shared<Database<SQLType>>(std::move(database));
    }
